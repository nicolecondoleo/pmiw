// Alumnas: Alessandra Angiolilo y Nicole Condoleo
// Comisión 1
// Link Youtube (Se mantiene el marcador de URL del video, si aplica)
...
// Array que contiene el texto de la narrativa para cada estado.
let frases = [
...
// ----------------------------------------------------------------------
// FUNCIÓN PRELOAD (Carga de assets)
function preload() {
...
// Caso especial: Estado 8 (Texto de varias líneas)
if (estado === 8) {
fill(255);      
textSize(20);   
textStyle(BOLD);
...
// Caso especial: Última pantalla (Créditos)
if (estado === 12) {
...
// Estado 2 (Los botones de decisión que te preocupan)
if (estado === 2) {
let botonAncho = 150; 
let botonAlto = 50;
let separacion = 30; 
...
// Dibuja la flecha de AVANCE (Solo si NO es estado de inicio, decisión o créditos).
...
// ----------------------------------------------------------------------
// FUNCIÓN MOUSEPRESSED (Interacciones)
...
// Caso Estado 12: VOLVER AL INICIO
if (estado === 12) {
if (finY === Y_FINAL_FIN) { 
...
// 🚩 Caso Estado 2: Botones de Decisión (CORRECTO: Asigna un nuevo estado y SALE)
if (estado === 2) {
let botonAncho = 150; 
let botonAlto = 50;
let separacion = 30; 
...
return; // ¡IMPORTANTE! Sale de la función, el avance general (flecha) no se procesa.
}
...
// Caso Estado 7: Botones de Decisión
if (estado === 7) {
let botonAncho = 220; 
let botonAlto = 50;
let separacion = 20; 
...
// Flujo de avance general (Flecha) - SÓLO SE PROCESA SI NINGÚN BOTÓN ANTERIOR FUE CLICADO
